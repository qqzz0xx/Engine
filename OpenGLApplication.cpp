...
{
	int result;
	auto colorBits = g_pAppConfig->redBits + g_pAppConfig->greenBits + g_pAppConfig->blueBits;
	...
	pfd.cAlphaBits = g_pAppConfig->alphaBits;
	pfd.cDepthBits = g_pAppConfig->depthBits;
	pfd.cStencilBits = g_pAppConfig->stencilBits;
	...
	if (GLAD_WGL_ARB_pixel_format && GLAD_WGL_ARB_multisample && GLAD_WGL_ARB_create_context)
	{
		...
		const int attributes[] = {
			...
			WGL_ALPHA_BITS_ARB,     g_pAppConfig->alphaBits,
			WGL_DEPTH_BITS_ARB,     g_pAppConfig->depthBits,
			WGL_STENCIL_BITS_ARB,   g_pAppConfig->stencilBits,
